    ...
    state={
     ...
     componentDidMount(){
        ...
        axios.get('/api/profile',{
            ...
            axios.get(`/api/edit/${response.data.username}/${link}`).then((response)=>{
                this.setState({
                    edits:response.data.reverse()
                })
            })
            axios.get(`/api/findstory/${response.data.username}/${link}`).then(
                (response)=>{
                    this.setState({
     ...
     handleDelete=(event)=>{
       ...
       axios.delete(`/api/edit/${event.target.edit.value}`,{
           data:{
               username:this.state.username
           }
       }).then(
           setTimeout(()=>{axios.get(`/api/edit/${this.state.username}/${this.props.match.params.story}`).then((response)=>{
               this.setState({
                   edits:response.data
               })
           })},500)
